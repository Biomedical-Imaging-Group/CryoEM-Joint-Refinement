/*
// Adjoint of 3D projection in parallel-beam geometry.
//
// cK:              KBWF expansion coeffs.
// proj:            The projections.
// r1, r2:          Projection coordinate.
// shiftX, shiftY:  In-plane translations
...
// supp :           Support of basis function in the projection domain
...
int numberOfSample, numberOfView, dimX, dimY;

typedef struct
{
    double *Ck, *proj, *Tp, *Ti, *r1, *r2, *shiftX, *shiftY;
    int    index       ;
    int    *N,*M       ;
} tdata;
                ...
                for (nx2=0; nx2<N[0]; nx2++) {
                    ...
                    for (ny1=ny1min; ny1<=ny1max; ny1++) {
                        for (ny2=ny2min; ny2<=ny2max; ny2++) {
                                Ck[(nx3*((int) N[1])+nx1)*((int) N[0])+nx2] += proj[i*((int) dimX*(int) dimY)+ny1*((int) dimX)+ny2] *Ti[0]* readLT((Tp[1]*(ny1-cy1)-y1)/Ti[0], (Tp[0]*(ny2-cy2)-y2)/Ti[0]);
                        }
                    }
                }
            }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs!=10) mexErrMsgTxt("There must be exactly 10 input arguments.");
    ...
    plhs[0] = mxCreateNumericArray(3, Nm, mxDOUBLE_CLASS, mxREAL);
    ...
    r1 = mxGetPr(prhs[2]);
    r2 = mxGetPr(prhs[3]);
    shiftX = mxGetPr(prhs[8]);
    shiftY = mxGetPr(prhs[9]);
    ...
    /*  Uncomment the line below if you don't want to use threading */
//     computeProjectionAdjoint((void*) data[0]);
    int rc;
    void * status;
