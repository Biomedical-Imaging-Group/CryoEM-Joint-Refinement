/*// g = projection3D(cK, r1,r2, Ti, Tp, LT, supp, shiftX, shiftY, scale)
//
...
typedef struct
{
    double *Ck, *proj, *r1, *r2, *Tp, *Ti, *shiftX, *shiftY;
    int    index       ;
    int    *N,*M       ;
} tdata;
...
double readLT(double u, double v, int projIndex) {
    ...
    if (r > supp) {
        ...
        int rmax = rmin+1    ;
        double p = r-rmin    ;
        return p * LT[rmax+projIndex*(LTSize[0])] + (1-p) * LT[rmin+projIndex*(LTSize[0])];
        /*int rround = round(r);
        //return LT[rmin]*/;
    }
}
...
void *computeProjection(void * t_data)
{
    ...
    double *Ck, *proj, *r1, *r2, *shiftX, *shiftY;
    double *Tp, *Ti;
    int    index       ;
    int    *N, *M      ;
    ...
    Tp  = (*th_data).Tp;
    Ti  = (*th_data).Ti;
    shiftX  = (*th_data).shiftX;
    shiftY  = (*th_data).shiftY;
    index= (*th_data).index;
    N  = (*th_data).N;
    M  = (*th_data).M;
        ...
        for (nx3=0; nx3<N[2]; nx3++) {
            ...
            for (nx1=0; nx1<N[1]; nx1++) {
                ...
                for (nx2=0; nx2<N[0]; nx2++) {
                    ...
                    y1 = x1nr1+x2nr1+x3nr1-shiftY[i];
                    y2 = x1nr2+x2nr2+x3nr2-shiftX[i];
                    ...
                    for (ny1=ny1min; ny1<=ny1max; ny1++) {
                        for (ny2=ny2min; ny2<=ny2max; ny2++) {
                                proj[i*((int) M[0]*(int) M[1])+ny1*((int) M[0])+ny2] += Ti[0]*Ck[(nx3*N[1]+nx1)*N[0]+nx2]* readLT((Tp[1]*(ny1-cy1)-y1)/Ti[0], (Tp[0]*(ny2-cy2)-y2)/Ti[0], i);
                        }
                    }
                }
            }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
    ...
    if (nrhs!=9) mexErrMsgTxt("There must be exactly 9 input arguments.");
    ...
    numberOfSample = mxGetDimensions(prhs[5])[0];
...
//     M[0] = ceil((p+2*supp*Ti[0])/Tp[0]);
    ...
    double p = N[0]; /*sqrt(N[0]*N[0]*Ti[0]*Ti[0]+N[1]*N[1]*Ti[1]*Ti[1]+N[2]*N[2]*Ti[2]*Ti[2]);*/
    double margin = 10;
    M[0] = ceil((p+2*margin)/Tp[0]);
    M[1] = ceil((p+2*margin)/Tp[1]);
    ...
    r2 = mxGetPr(prhs[2]);
    LT = mxGetPr(prhs[5]);
    shiftX = mxGetPr(prhs[7]);
    shiftY = mxGetPr(prhs[8]);
    
    LTSize[0] = mxGetDimensions(prhs[5])[0];
    LTSize[1] = mxGetDimensions(prhs[5])[1];
    mexPrintf("\n first dimension is %d, second dimension is %d.\n",LTSize[0],LTSize[1]);
    ...
    for (t=0 ; t<NTHREAD ; t++){
        ...
        (*data[t]).Tp     =  Tp  ;
        (*data[t]).Ti     =  Ti  ;
        (*data[t]).shiftX =  shiftX  ;
        (*data[t]).shiftY =  shiftY  ;
        (*data[t]).index  =  t   ;
        (*data[t]).N      =  (int *)N;
        (*data[t]).M      =  (int *)M;
