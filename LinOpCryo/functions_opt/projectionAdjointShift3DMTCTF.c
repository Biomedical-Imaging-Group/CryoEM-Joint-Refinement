                ...
                for (nx2=0; nx2<N[0]; nx2++) {
                    ...
                    for (ny1=ny1min; ny1<=ny1max; ny1++) {
                        for (ny2=ny2min; ny2<=ny2max; ny2++) {
                                Ck[(nx3*((int) N[1])+nx1)*((int) N[0])+nx2] += proj[i*((int) M[0]*(int) M[1])+ny1*((int) M[0])+ny2] *Ti[0]* readLT((Tp[1]*(ny1-cy1)-y1)/Ti[0], (Tp[0]*(ny2-cy2)-y2)/Ti[0], i);
                        }
                    }
                }
            }
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) {
   ...
   /*/ mexPrintf("\n M size is %d ",mxGetNumberOfElements(prhs[2]));*/
    ...
    numberOfSample = mxGetDimensions(prhs[6])[0];
    ...
    LTSize[0] = mxGetDimensions(prhs[6])[0];
    LTSize[1] = mxGetDimensions(prhs[6])[1];
